import MimoxiAudio from "./Audio.js";
...
export default class Mimoxi {
  constructor() {
    ...
    */
    ...
    const init = (event) => {
      this.audio = new MimoxiAudio();
      this.keys = [0, 1, 2, 3, 4, 5, 6, 7].map(
        (x, index) =>
          new MimoxiKey(
            document.getElementById("key" + index),
            this.audio,
            792 * (index + 1)
          )
      );
  ...
  pattern() {
    ...
    for (let i = 0; i < pattern.length; i++) {
      setTimeout(()=>{
        this.keys[i].play();
        setTimeout(()=>this.keys[i].stop(), this.beat*(i+1) + i);
      }, this.beat*(i+1));
    }
  }
  
  soundOn(){
    this.audio.mainGainNode.gain.cancelScheduledValues( this.audio.context.currentTime );
    this.audio.mainGainNode.gain.cancelAndHoldAtTime( this.audio.context.currentTime );
  ...
  bindButtons() {
    this.buttons = {
      ...
      sound: new MimoxiButton(document.getElementById("sound_btn_svg")),
    };
    this.buttons["mode"].bind("on", (e) => this.hello(()=>this.pattern()));
    this.buttons["sound"].bind("on", (e) => this.soundOn());
    this.buttons["sound"].bind("off", (e) => this.audio.context.suspend());
  }
