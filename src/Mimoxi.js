...
export default class Mimoxi {
  constructor () {
    ...
    const init = (event) => {
      ...
      keyPress()
        ? this.awakenFrom(event.target, bind)
        : this.awaken(bind)
        ...
        nextMap = this.keys.map((key, index) => {
          switch (key.element.getAttribute(
            ...
            default: // no class = dim
              return this.hasBrightNeighbor(index)
                ? 'on'
                : ''
          }
        })
  ...
  bindButtons () {
    this.buttons = {
      mode: new MimoxiModeButton(this, document
        .getElementById(
          'mode_btn_svg')),
      mute: new MimoxiMuteButton(this, document
        .getElementById(
          'mute_btn_svg'))
    }
  }
  ...
  bindKeys () {
    ...
    this.keys.forEach(key => {
      this.controls.push(new MimoxiControl(this,
        key.element)
        .bind('mousedown', play)
        .bind('mouseup', stop)
